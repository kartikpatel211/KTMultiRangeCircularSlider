...
class DoubleHandleCircularSlider: CircularSlider {
  ...
  fileprivate var upperAngleFromNorth: CGFloat = 30 {
    didSet {
  ...
  fileprivate func moveLowerHandle(_ newAngleFromNorth: CGFloat) {
    ...
    guard !conditionOne && !conditionTwo else {
      ...
      if conditionOne && conditionTwo && angleFromNorth < maximumAngle / 2 {
        angleFromNorth = 0
      // prevent handles from crossing
      } else if conditionTwo {
        angleFromNorth = upperAngleFromNorth - minimumHandleDistance
      }
  ...
  fileprivate func moveUpperHandle(_ newAngleFromNorth: CGFloat) {
    let conditionOne = newAngleFromNorth > maximumAngle
    let conditionTwo = newAngleFromNorth < angleFromNorth + minimumHandleDistance
    
    guard !conditionOne && !conditionTwo else {
      // prevent slider from moving past maximumAngle
      if conditionOne && upperAngleFromNorth > maximumAngle / 2 {
        upperAngleFromNorth = maximumAngle
        // prevent handles from crossing
      } else if conditionTwo {
        upperAngleFromNorth = angleFromNorth + minimumHandleDistance
      }
    ...
    upperAngleFromNorth = newAngleFromNorth
    setNeedsDisplay()
  }
